...
async function fetchLineupsFromApi(matchId: string) {
  const apiUrl = `https://spl.torneopal.net/taso/rest/getMatch?match_id=${matchId}`;
  ...
  try {
    ...
    throw new Error(`Match API returned non-JSON: ${text.slice(0, 500)}`);
  }
  ...
  function toPlayer(p: any, fallbackPrefix: string, i: number): LineupPlayer {
    ...
    return {
      spl_player_id:
        p.player_id != null ? String(p.player_id) : `${fallbackPrefix}-${i}`,
      name: fullName,
      shirt_no: shirtNo,
    };
  }
  ...
  return {
    ...
    matchMeta: {
      home_score:
        match.fs_A != null && match.fs_A !== "" ? Number(match.fs_A) : null,
      away_score:
        match.fs_B != null && match.fs_B !== "" ? Number(match.fs_B) : null,
      kickoff_at:
      ...
      competition: {
        gender:
          String(match.category_group_name ?? "").toLowerCase().includes("naiset")
            ? "Female"
            : String(match.category_group_name ?? "")
                .toLowerCase()
                .includes("miehet")
              ? "Male"
              : null,
    ...
    if (lineupRows && lineupRows.length > 0) {
      ...
      const teams: TeamsBlock = {
        home: {
          ...
          team_name: homeTeamSplId
            ? (teamNameById.get(homeTeamSplId) ?? null)
            : null,
        },
        away: {
          spl_team_id: awayTeamSplId,
          team_name: awayTeamSplId
            ? (teamNameById.get(awayTeamSplId) ?? null)
            : null,
        },
      };
      ...
      const homeStarters = uniqById(
        lineupRows
          .filter((r: any) => r.side === "home" && r.squad === "xi")
          .map(rowToPlayer),
      );
      const homeBench = uniqById(
        lineupRows
          .filter((r: any) => r.side === "home" && r.squad === "bench")
          .map(rowToPlayer),
      );
      const awayStarters = uniqById(
        lineupRows
          .filter((r: any) => r.side === "away" && r.squad === "xi")
          .map(rowToPlayer),
      );
      const awayBench = uniqById(
        lineupRows
          .filter((r: any) => r.side === "away" && r.squad === "bench")
          .map(rowToPlayer),
      );
